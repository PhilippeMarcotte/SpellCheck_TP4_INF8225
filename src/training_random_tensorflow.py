...
class DataReader:
    ...
    def __init__(self, word_tensor, char_tensor, batch_size, num_unroll_steps, char_vocab):
        self.char_tokens = char_vocab.tokenByIndex_
    ...
    def random_position(self, array):
       return tf.random_uniform(shape=(1,), minval=0, maxval=len(array), dtype=tf.int32)

    def replace_random_character(self, word):
        random_char_position = self.random_position(word)
        word[random_char_position.eval()] = self.random_position(self.char_tokens).eval()
        return word
    
    def delete_random_characeter(self, word):
        random_char_position = self.random_position(word).eval()
        word = word[:random_char_position] + word[random_char_position + 1:]
        return word

    def add_random_character(self, word):
        random_char_position = self.random_position(word).eval()
        random_char = random_position(self.char_tokens).eval()
